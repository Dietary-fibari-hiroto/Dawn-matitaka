...
const Header = () => {
    const routes = [
        ...
        { name: "Dawn-myportfolio", path: "https://dawn-waiting.com" }
    ]
    ...
    return (
        <div className="palast-variable absolute z-[50] flex items-center justify-between h-[50px] w-[100vw] px-[10px]">
            <Link to="/" className="pl-[10px]">
                <img className="w-[50px] h-[50px]" src={ImagesRoutes.matitaka_wh} alt="matitaka_log" />
            </Link>
            <ul className="flex items-center space-x-[50px] text-white pr-[10px] max-sm:hidden">
                {
                    routes.map(({ name, path }) => (
                        <li key={name} className="border-l pl-[10px]">
            ...
            {isOpen && (
                <div className="fixed section-frame top-0 right-0 w-48 bg-white rounded-md shadow-lg z-10">
                    ...
                    <ul className="py-2 text-gray-800 text-[40px]">
                        {
                            routes.map(({ name, path }) => (
                                <li key={name} className="px-4 py-2 hover:bg-gray-100 cursor-pointer">
                                    <Link to={path}>{name}</Link>
                                </li>
                            ))
                        }
